...
load_dotenv()  # admin_test_app/.env 파일이 있으면 여기서 자동으로 읽어서 os.environ에 채워준다
...
# bene_ai에서 이미 쓰고 있는 모델이 따로 있으면 이 두 환경변수로 그걸 그대로 지정하면 된다.
...
EXTRACT_EXCLUDE_FIELDS = {"source", "source_id", "raw"}
